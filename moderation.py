...
def _load_threat():
    ...
    if not YOLO_WEIGHTS.is_file():
        raise FileNotFoundError(f"missing {YOLO_WEIGHTS} - run download_models.py")
...
def _load_ocr():
    ...
    if not (TROCR_DIR / "model.safetensors").is_file():
        raise FileNotFoundError(f"missing {TROCR_DIR} - run download_models.py")
    processor = TrOCRProcessor.from_pretrained(str(TROCR_DIR))
    model = VisionEncoderDecoderModel.from_pretrained(str(TROCR_DIR))
    model.eval()
